...
/*
    ...
    */
    ...
    while(1){
        ...
        if(client[0].revents & POLLIN)//判定哪一个套接口发生了事件。判断监听套接口是否在集合当中。
        {
        ...
        for(int i = 1; i<=maxi; i++){//select还有可能是conn事件
            printf("maxi = %d\n", maxi);
            conn = client[i].fd;//判断是否待处理
            if(conn == -1)
                continue;
            if(client[i].events & POLLIN){//判断是否在集合中
                char recvbuf[1024] = {0};
